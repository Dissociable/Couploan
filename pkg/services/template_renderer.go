...
func (t *TemplateRenderer) RenderPage(ctx fiber.Ctx, page page.Page) error {
	...
	if page.Name == "" {
		return fiber.NewError(http.StatusInternalServerError, "page render failed due to missing name")
	}
	...
	if err != nil {
		...
		logger.Error("failed to parse and execute templates", zap.Error(err))
...
func (t *TemplateRenderer) cachePage(ctx fiber.Ctx, page page.Page, html *bytes.Buffer) {
	...
	case !context.IsCanceledError(err):
		l := util.GetLoggerFromFiberCtx(ctx)
		if l != nil {
			l.Error("failed to cache page", zap.Error(err))
		}
	}
}

// GetCachedPage attempts to fetch a cached page for a given URL
func (t *TemplateRenderer) GetCachedPage(ctx fiber.Ctx, url string) (*CachedPage, error) {
	p, err := t.cache.
		Get().
		Group(cachedPageGroup).
		Key(url).
