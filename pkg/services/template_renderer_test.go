...
func TestTemplateRenderer_RenderPage(t *testing.T) {
	//setup := func() (fiber.Ctx, *httptest.ResponseRecorder, page.Page) {
	//	ctx, rec := tests.NewContextTest(c.Web, "/test/TestTemplateRenderer_RenderPage", nil)
	//
	//	p := page.New(ctx)
	//	p.Name = "home"
	//	p.Layout = "main"
	...
	//
	//t.Run(
	//	"no page cache", func(t *testing.T) {
	//		ctx, _, p := setup()
	//		err := c.TemplateRenderer.RenderPage(ctx, p)
	//		require.NoError(t, err)
	//
	...
	//
	//t.Run(
	//	"htmx rendering", func(t *testing.T) {
	//		ctx, _, p := setup()
